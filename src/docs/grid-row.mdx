...
export const description = "Utilidades para controlar cómo se dimensionan y colocan los elementos a lo largo de las filas de la cuadrícula.";
...
Utiliza utilidades `row-span-<number>` como `row-span-2` y `row-span-4` para hacer que un elemento se expanda _n_ filas:
...
Utiliza utilidades `row-start-<number>` o `row-end-<number>` como `row-start-2` y `row-end-3` para hacer que un elemento comience o termine en la _n-ésima_ línea de la cuadrícula:
...
Estas también se pueden combinar con las utilidades `row-span-<number>` para expandir un número específico de filas.
...
<UsingACustomValue
  ...
  variable="rows"
  name="el tamaño y ubicación de las filas de cuadrícula"
