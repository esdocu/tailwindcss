...
export const description = "Utilidades para controlar la colocación de elementos posicionados.";
...
Usa utilidades `top-<number>`, `right-<number>`, `bottom-<number>`, `left-<number>` e `inset-<number>` como `top-0` y `bottom-4` para establecer la posición horizontal o vertical de un [elemento posicionado](/docs/position):
...
Usa utilidades `inset-s-<number>` o `inset-e-<number>` como `inset-s-0` e `inset-e-4` para establecer las [propiedades lógicas](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts) `inset-inline-start` e `inset-inline-end`, que se asignan al lado izquierdo o derecho según la dirección del texto:
...
Para un mayor control, también puedes usar los [modificadores LTR y RTL](/docs/hover-focus-and-other-states#rtl-support) para aplicar estilos específicos de forma condicional según la dirección actual del texto.

### Usar un valor personalizado

<UsingACustomValue element="div" utility="top" name="position" value="3px" />
