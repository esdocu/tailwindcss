...
Utiliza utilidades `max-w-<number>` como `max-w-24` y `max-w-64` para establecer un ancho máximo fijo para un elemento según la escala de espaciado:

<Figure hint="Cambia el tamaño del ejemplo para ver el comportamiento esperado">
...
Utiliza las utilidades `max-w-full` o `max-w-<fraction>` como `max-w-1/2` y `max-w-2/5` para dar a un elemento un ancho máximo basado en porcentaje:
...
Utiliza utilidades como `max-w-sm` y `max-w-xl` para establecer un ancho máximo fijo para un elemento según la escala de contenedores:
    ...
    <div className="dark:highlight-white/5 relative mx-auto flex max-w-md items-center gap-6 overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-black/5 dark:bg-gray-800">
      ...
      <div className="min-w-0 py-5 pr-5 pl-28">
        ...
        <div className="truncate text-sm leading-tight text-gray-500 sm:text-base dark:text-gray-400">
          Asistente del Secretario de Viajes
        </div>
      </div>
    </div>
  }
...
### Usar el contenedor de puntos de interrupción

Utiliza la utilidad `container` para establecer el ancho máximo de un elemento de modo que coincida con el `min-width` del punto de interrupción actual. Esto es útil si prefieres diseñar para un conjunto fijo de tamaños de pantalla en lugar de intentar adaptarte a un viewport totalmente fluido:
...
Ten en cuenta que, a diferencia de los contenedores que habrás utilizado en otros frameworks, el contenedor de Tailwind no se centra automáticamente y no tiene ningún relleno horizontal integrado. Utiliza las utilidades `mx-auto` y `px-<number>` para añadirlos:
...
<UsingACustomValue utility="max-w" value="220px" name="ancho máximo" variable="max-width" />
