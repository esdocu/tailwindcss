...
Utiliza utilidades `border` o `border-<number>` como `border-2` y `border-4` para establecer el ancho del borde en todos los lados de un elemento:
...
Utiliza utilidades como `border-r` y `border-t-4` para establecer el ancho del borde en un lado de un elemento:
...
Utiliza utilidades como `border-x` y `border-y-4` para establecer el ancho del borde en dos lados de un elemento al mismo tiempo:
...
Utiliza utilidades como `border-s` y `border-e-4` para establecer las propiedades lógicas `border-inline-start-width` y `border-inline-end-width` [propiedades lógicas](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts), que se asignan al borde izquierdo o derecho según la dirección del texto:
...
Utiliza las utilidades `border-bs-<number>` y `border-be-<number>` para establecer las propiedades lógicas `border-block-start-width` y `border-block-end-width`, que se asignan al borde superior o inferior según el modo de escritura:
...
Utiliza utilidades como `divide-x` y `divide-y-4` para añadir bordes entre elementos hijos:
...
Si tus elementos están en orden inverso (usando por ejemplo `flex-row-reverse` o `flex-col-reverse`), utiliza las utilidades `divide-x-reverse` o `divide-y-reverse` para asegurarte de que el borde se agregue al lado correcto de cada elemento:
...
<UsingACustomValue utility="border" name="ancho del borde" value="2vw" dataType="length" variable="border-width" />
