...
export const description = "Utilidades para controlar cómo se dimensionan y colocan los elementos a lo largo de las columnas de la cuadrícula.";
...
Utiliza utilidades `col-span-<number>` como `col-span-2` y `col-span-4` para hacer que un elemento se expanda _n_ columnas:
...
Utiliza utilidades `col-start-<number>` o `col-end-<number>` como `col-start-2` y `col-end-3` para hacer que un elemento comience o termine en la _n-ésima_ línea de la cuadrícula:
...
Estas también se pueden combinar con las utilidades `col-span-<number>` para expandir un número específico de columnas.
...
<UsingACustomValue
  ...
  variable="columns"
  name="el tamaño y ubicación de las columnas de cuadrícula"
  value="16_/_span_16"
/>
