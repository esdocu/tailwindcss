...
export const description = "Utilidades para controlar el número de columnas dentro de un elemento.";
...
Utiliza utilidades `columns-<number>` como `columns-3` para definir el número de columnas que se deben crear para el contenido dentro de un elemento:
...
Utiliza utilidades como `columns-xs` y `columns-sm` para definir el ancho ideal de columna para el contenido dentro de un elemento:

<Figure desktopHint="Cambia el tamaño del ejemplo para ver el comportamiento esperado">
...
Al establecer el ancho de la columna, el número de columnas se ajusta automáticamente para garantizar que no sean demasiado estrechas.

### Definir la separación de columnas (gap)

Utiliza las utilidades `gap-<width>` para especificar el ancho entre las columnas:
...
Obtén más información sobre las utilidades de separación en la [documentación de gap](/docs/gap).
