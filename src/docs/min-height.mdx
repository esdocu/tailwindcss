...
Utiliza utilidades `min-h-<number>` como `min-h-24` y `min-h-64` para establecer una altura mínima fija para un elemento según la escala de espaciado:
...
Utiliza las utilidades `min-h-full` o `min-h-<fraction>` como `min-h-1/2` y `min-h-2/5` para dar a un elemento una altura mínima basada en porcentaje:
...
<UsingACustomValue utility="min-h" value="220px" name="altura mínima" variable="min-height" />
