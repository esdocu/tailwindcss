...
Usa utilidades `max-h-<number>` como `max-h-24` y `max-h-64` para establecer una altura máxima fija para un elemento basada en la escala de espaciado:
...
Usa utilidades `max-h-full` o `max-h-<fraction>` como `max-h-1/2` y `max-h-2/5` para darle a un elemento una altura máxima basada en porcentajes:
