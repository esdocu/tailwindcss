...
export const description = "Utilidades para establecer la altura máxima de un elemento.";
...
Utiliza utilidades `max-h-<number>` como `max-h-24` y `max-h-64` para establecer una altura máxima fija para un elemento según la escala de espaciado:
...
Utiliza las utilidades `max-h-full` o `max-h-<fraction>` como `max-h-1/2` y `max-h-2/5` para dar a un elemento una altura máxima basada en porcentaje:
...
<UsingACustomValue utility="max-h" value="220px" name="altura máxima" variable="max-height" />
