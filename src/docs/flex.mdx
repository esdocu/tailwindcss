...
export const description = "Utilidades para controlar cómo crecen y se encogen los elementos flex.";
...
Utiliza utilidades `flex-<number>` como `flex-1` para permitir que un elemento flex crezca y se encoja según sea necesario, ignorando su tamaño inicial:
...
Utiliza `flex-initial` para permitir que un elemento flex se encoja pero no crezca, teniendo en cuenta su tamaño inicial:
...
Utiliza `flex-auto` para permitir que un elemento flex crezca y se encoja, teniendo en cuenta su tamaño inicial:
...
Utiliza `flex-none` para evitar que un elemento flex crezca o se encoja:
...
<UsingACustomValue utility="flex" name="propiedad abreviada flex" value="3_1_auto" />
