...
Usa utilidades como `border-rose-500` y `border-lime-100` para controlar el color del borde de un elemento:
...
Usa el modificador de opacidad de color para controlar la opacidad del color del borde de un elemento:
...
Usa utilidades como `border-t-indigo-500` y `border-r-lime-100` para establecer el color del borde para un lado de un elemento:
...
Usa utilidades como `border-x-indigo-500` y `border-y-lime-100` para establecer el color del borde en dos lados de un elemento al mismo tiempo:
...
Usa utilidades como `border-s-indigo-500` y `border-e-lime-100` para establecer las [propiedades lógicas](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts) `border-inline-start-color` y `border-inline-end-color`, que se asignan al borde izquierdo o derecho según la dirección del texto:
...
Usa utilidades como `divide-indigo-500` y `divide-lime-100` para controlar el color del borde entre elementos hijos:
