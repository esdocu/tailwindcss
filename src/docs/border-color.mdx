...
export const description = "Utilidades para controlar el color de los bordes de un elemento.";
...
Utiliza utilidades como `border-rose-500` y `border-lime-100` para controlar el color del borde de un elemento:
...
Utiliza el modificador de opacidad de color para controlar la opacidad del color del borde de un elemento:
...
Utiliza utilidades como `border-t-indigo-500` y `border-r-lime-100` para establecer el color del borde de un lado de un elemento:
...
Utiliza utilidades como `border-x-indigo-500` y `border-y-lime-100` para establecer el color del borde en dos lados de un elemento al mismo tiempo:
...
Utiliza utilidades como `border-s-indigo-500` y `border-e-lime-100` para establecer las propiedades lógicas `border-inline-start-color` y `border-inline-end-color` [propiedades lógicas](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts), que se asignan al borde izquierdo o derecho según la dirección del texto:
...
Utiliza las utilidades `border-bs-*` y `border-be-*` para establecer las propiedades lógicas `border-block-start-color` y `border-block-end-color`, que se asignan al borde superior o inferior según el modo de escritura:
...
### Divisor entre elementos hijos

Utiliza utilidades como `divide-indigo-500` y `divide-lime-100` para controlar el color del borde entre los elementos hijos:
...
<UsingACustomValue utility="border" name="color del borde" value="#243c5a" />
...
<Example>
  {
    <label className="mx-auto block max-w-xs">
      <span className="text-sm font-medium text-gray-900 dark:text-gray-200">Dirección de correo electrónico</span>
      <input
        type="text"
