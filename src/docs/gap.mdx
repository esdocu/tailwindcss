...
export const description = "Utilidades para controlar los canales (gutters) entre elementos de cuadrícula y flexbox.";
...
Utiliza utilidades `gap-<number>` como `gap-2` y `gap-4` para cambiar el espacio entre filas y columnas en diseños de cuadrícula (grid) y flexbox:
...
### Cambiar el espacio de filas y columnas de forma independiente

Utiliza utilidades `gap-x-<number>` o `gap-y-<number>` como `gap-x-8` y `gap-y-4` para cambiar el espacio entre columnas y filas de forma independiente:
