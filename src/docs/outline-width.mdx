...
export const description = "Utilidades para controlar el grosor del contorno de un elemento.";
...
Utiliza utilidades `outline` o `outline-<number>` como `outline-2` y `outline-4` para establecer el grosor del contorno de un elemento:
...
<button class="outline outline-offset-2 ...">Botón A</button>
<button class="outline-2 outline-offset-2 ...">Botón B</button>
<button class="outline-4 outline-offset-2 ...">Botón C</button>
```
...
<Figure hint="Enfoca el botón para ver el contorno añadido">
...
<button class="outline-offset-2 outline-sky-500 focus:outline-2 ...">Guardar cambios</button>
```
...
<UsingACustomValue utility="outline" value="2vw" name="grosor del contorno" dataType="length" variable="outline-width" />
