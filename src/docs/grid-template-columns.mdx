...
### Especificar las columnas de cuadrícula

Utiliza utilidades `grid-cols-<number>` como `grid-cols-2` y `grid-cols-4` para crear cuadrículas con _n_ columnas de igual tamaño:
...
### Implementar una subcuadrícula (subgrid)

Utiliza la utilidad `grid-cols-subgrid` para adoptar las pistas de columnas definidas por el elemento padre:
...
<UsingACustomValue utility="grid-cols" name="columnas" value="200px_minmax(900px,_1fr)_100px" />
