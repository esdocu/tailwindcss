...
Usa utilidades `w-<number>` como `w-24` y `w-64` para establecer un ancho fijo para un elemento basándote en la escala de espaciado:
...
Usa las utilidades `w-full` o `w-<fraction>` como `w-1/2` y `w-2/5` para darle a un elemento un ancho basado en un porcentaje:
...
Usa utilidades como `w-sm` and `w-xl` para establecer un ancho fijo para un elemento basándote en la escala de contenedores:
...
Usa la utilidad `w-screen` para hacer que un elemento ocupe todo el ancho del viewport:
...
Alternativamente, puedes coincidir con el ancho de los viewports grandes, pequeños o dinámicos usando las utilidades `w-lvw`, `w-svw` y `w-dvw`.

### Restablecer el ancho

Usa la utilidad `w-auto` para eliminar el ancho asignado a un elemento bajo una condición específica, como en un punto de ruptura particular:
...
### Establecer tanto el ancho como la altura
...
<UsingACustomValue utility="w" value="5px" name="ancho" variable="width" />
