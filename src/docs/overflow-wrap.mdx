...
export const description = "Utilidades para controlar los saltos de línea dentro de palabras en un elemento que se desborda.";
...
### Ajustar a mitad de palabra

Utiliza la utilidad `wrap-break-word` para permitir saltos de línea entre letras de una palabra si es necesario:
...
<Example padding={false}>
  {
    <p className="mx-auto max-w-xs border-x border-x-pink-400/30 py-8 wrap-break-word text-gray-900 dark:text-gray-200">
      La palabra más larga en cualquiera de los principales diccionarios de la lengua inglesa es{" "}
      <span className="font-bold">pneumonoultramicroscopicsilicovolcanoconiosis,</span> una palabra que se refiere a una
      enfermedad pulmonar contraída por la inhalación de partículas de sílice muy finas, específicamente de un volcán;
      médicamente, es lo mismo que la silicosis.
    </p>
  }
</Example>
...
### Ajustar en cualquier lugar

La utilidad `wrap-anywhere` se comporta de manera similar a `wrap-break-word`, excepto que el navegador tiene en cuenta los saltos de línea a mitad de palabra al calcular el tamaño intrínseco del elemento:
...
Esto es útil para ajustar texto dentro de contenedores `flex`, donde normalmente necesitarías establecer `min-width: 0` en el elemento hijo para permitir que se encoja por debajo del tamaño de su contenido.

### Ajustar normalmente

Utiliza la utilidad `wrap-normal` para permitir únicamente saltos de línea en puntos de ajuste naturales, como espacios, guiones y signos de puntuación:
