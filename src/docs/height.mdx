...
export const description = "Utilidades para establecer la altura (height) de un elemento.";
...
Utiliza utilidades `h-<number>` como `h-24` y `h-64` para establecer una altura fija en un elemento basada en la escala de espaciado:
...
Utiliza utilidades `h-full` o `h-<fraction>` como `h-1/2` y `h-2/5` para dar a un elemento una altura basada en porcentaje:
...
Utiliza la utilidad `h-screen` para hacer que un elemento ocupe toda la altura del viewport:
...
### Ajustar al viewport dinámico

Utiliza la utilidad `h-dvh` para hacer que un elemento ocupe toda la altura del viewport dinámico, la cual cambia a medida que la interfaz del navegador se expande o se contrae:

<Figure hint="Desplaza el viewport para ver cambiar la altura del viewport">
...
### Ajustar al viewport grande

Utiliza la utilidad `h-lvh` para establecer la altura de un elemento al valor máximo posible de la altura del viewport:
...
### Ajustar al viewport pequeño

Utiliza la utilidad `h-svh` para establecer la altura de un elemento al valor mínimo posible de la altura del viewport:
...
### Establecer ancho y alto al mismo tiempo

Utiliza utilidades como `size-px`, `size-4`, y `size-full` para establecer tanto el ancho como la altura de un elemento al mismo tiempo:
...
<UsingACustomValue utility="h" value="32rem" name="altura" variable="height" />
