...
export const description = "Utilidades para establecer el ancho mínimo de un elemento.";
...
Utiliza utilidades `min-w-<number>` como `min-w-24` y `min-w-64` para establecer un ancho mínimo fijo para un elemento según la escala de espaciado:
...
Utiliza las utilidades `min-w-full` o `min-w-<fraction>` como `min-w-1/2` y `min-w-2/5` para dar a un elemento un ancho mínimo basado en porcentaje:
...
Utiliza utilidades como `min-w-sm` y `min-w-xl` para establecer un ancho mínimo fijo para un elemento según la escala de contenedores:
...
<UsingACustomValue utility="min-w" value="220px" name="ancho mínimo" variable="min-width" />
