...
Usa utilidades `min-w-<number>` como `min-w-24` y `min-w-64` para establecer un ancho mínimo fijo para un elemento basada en la escala de espaciado:
...
Usa utilidades `min-w-full` o `min-w-<fraction>` como `min-w-1/2` y `min-w-2/5` para darle a un elemento un ancho mínimo basado en porcentajes:
...
Usa utilidades como `min-w-sm` y `min-w-xl` para establecer un ancho mínimo fijo para un elemento basado en la escala del contenedor:
